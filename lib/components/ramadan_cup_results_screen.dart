...
class PremierePhaseRamadanResultsScreen extends StatefulWidget {
  final int league;
  final Function(Match) onMatchSelected;

  const PremierePhaseRamadanResultsScreen({
    Key? key,
  ...
  @override
  _PremierePhaseRamadanResultsScreenState createState() =>
      _PremierePhaseRamadanResultsScreenState();
}

class _PremierePhaseRamadanResultsScreenState
    extends State<PremierePhaseRamadanResultsScreen> {
  ...
  void _groupMatchesByPool() {
    poolMatches = {}; // Reset the poolMatches map

    for (var pool in poolData) {
      if (pool.playedMatches != null && pool.playedMatches!.isNotEmpty) {
        poolMatches[pool.pool?.name ?? "Unnamed Pool"] = pool.playedMatches!;
      }
    }

    print("Grouped Matches by Pool: $poolMatches");
  }
