...
class RamadanCupCalendar extends StatefulWidget {
  final int seasonId; // Accept seasonId as a parameter

  const RamadanCupCalendar({
    Key? key,
    required this.seasonId, // Required seasonId
  }) : super(key: key);

  @override
  _RamadanCupCalendarState createState() => _RamadanCupCalendarState();
}

class _RamadanCupCalendarState extends State<RamadanCupCalendar> {
  ...
  Future<void> _fetchBracketData() async {
    try {
      ...
      for (var bracketData in bracketDataList) {
        ...
        // Add the semi-finals and finals from each bracket
        setState(() {
  ...
  // Organize matches by phase and filter where is_ended = false
  Map<String, List<Match>> matchesByPhase() {
    return {
      'Demi-Finals': semiFinals
          .where((match) => !match.is_ended!) // Filter where is_ended = false
          .toList(),
      'Finals': finals
          .where((match) => !match.is_ended!) // Filter where is_ended = false
          .toList(),
